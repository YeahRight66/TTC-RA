...
ConVar mat_noise_scale("mat_noise_scale", "2.0");//was 4.2

BEGIN_VS_SHADER( Post_NightVision, "Help for Post_NightVision" )

	BEGIN_SHADER_PARAMS
	SHADER_PARAM( NVLEVEL,			SHADER_PARAM_TYPE_FLOAT,	"2.5",				"")
	SHADER_PARAM(INNER_RADIUS,		SHADER_PARAM_TYPE_FLOAT, "38", "")
	SHADER_PARAM(OUTER_RADIUS,		SHADER_PARAM_TYPE_FLOAT, "45", "")
	SHADER_PARAM( FBTEXTURE,		SHADER_PARAM_TYPE_TEXTURE,	"_rt_FullFrameFB",	"" )
	SHADER_PARAM( NOISETEXTURE,		SHADER_PARAM_TYPE_TEXTURE,	"shaders/test_noise",					"Noise texture" )
	SHADER_PARAM( NOISESCALE,		SHADER_PARAM_TYPE_FLOAT,	"1",				"Noise scale" )
	END_SHADER_PARAMS
	...
	SHADER_INIT_PARAMS()
	{
		...
		{
			params[ NOISETEXTURE ]->SetStringValue( "shaders/test_noise" );
		}
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			if (mat_noise_scale.GetFloat() != 0.0f)
				flNoiseScale = mat_noise_scale.GetFloat();
